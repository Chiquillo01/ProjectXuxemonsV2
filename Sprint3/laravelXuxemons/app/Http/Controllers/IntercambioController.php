...
class IntercambioController extends Controller
{
    ...
    public function xuxemonsIntercambio1($userToken, $idUser)
    {
        try {
            // $userToken = $request->input('token');
            // $idUser = $request->input('iduser2');
            ...
            $idUser2 = $user->idUser;
            $infoTrade = Intercambio::where('user2', $idUser2)->where('user1', $idUser)->first();
            ...
            $tradeUser1 = XuxemonsUser::where('user_id', $idUser)->where('xuxemon_id', $idXuxemon1)
                ->join('xuxemons', 'xuxemons_users.xuxemon_id', '=', 'xuxemons.id')
                ->select(
                    'xuxemons_users.*',
                    'xuxemons.nombre',
    ...
    public function xuxemonsIntercambio2($userToken, $idUser)
    {
        try {
            // $userToken = $request->input('token');
            // $idUser = $request->input('iduser2');
            ...
            $tradeUser2 = XuxemonsUser::where('user_id', $idUser2)->where('xuxemon_id', $idXuxemon2)
                ->join('xuxemons', 'xuxemons_users.xuxemon_id', '=', 'xuxemons.id')
                ->select(
                    'xuxemons_users.*',
                    'xuxemons.nombre',
            ...
            return response()->json($tradeUser2, 200);
        } catch (\Exception $e) {
            return response()->json(['message' => 'Ha ocurrido un error al retornar las solicitudes: ' . $e->getMessage()], 500);
        }
    }
